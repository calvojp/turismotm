...
public class ItinerarioDAO {
	...
	// 	para que arroje nombre y atraccion //

	public int insert(Itinerario itinerario) throws SQLException {
		String sql = "INSERT INTO itinerarios (id_username,  id_atraccion) VALUES (?, ?)";
	...
	// actualice y haga la connection

	public int update(Itinerario itinerario) throws SQLException {
		String sql = "UPDATE ITINERARIOS SET  id_atraccion=?,  WHERE id_username = ?";
		Connection conn = ConnectionProvider.getConnection();
	...
	public List<Itinerario> findByUser(int idUsuario) throws SQLException {
		String sql = "SELECT * FROM itinerarios WHERE id_username=?";
		Connection conn = ConnectionProvider.getConnection();
		PreparedStatement statement = conn.prepareStatement(sql);
